...
for c1, c2 in itertools.combinations(gapminder.columns, 2):
    pearson_r, p_value_two_tais = scipy.stats.pearsonr(gapminder[c1], gapminder[c2])
    list_of_max_pearsons.append((pearson_r, c1, c2))
list_of_max_pearsons.sort(key=lambda x: x[0], reverse=True)

# taking some interesting values
pearson_r_max, c1_max, c2_max = list_of_max_pearsons[0]
pearson_r_max_2, c1_max_2, c2_max_2 = list_of_max_pearsons[1]
pearson_r_zero, c1_zero, c2_zero = list_of_max_pearsons[70]
pearson_r_min, c1_min, c2_min = list_of_max_pearsons[-1]
...
plt.xlabel(c1_max)
plt.ylabel(c2_max)
plt.text(20000, 0, (r'$r_{{pearson}}={0:0.3f}${2}$r_{{pearson}}^2={1:0.3f}$').format(
...
plt.xlabel(c1_max_2)
plt.ylabel(c2_max_2)
plt.text(0, 6000, (r'$r_{{pearson}}={0:0.3f}${2}$r_{{pearson}}^2={1:0.3f}$').format(
...
plt.close(1)
print('pearsons r: {0:0.3f}\npearsons rÂ²: {1:0.3f}\nfor {2} and {3} data sets'.format(
pearson_r_max, pearson_r_max ** 2, c1_max, c2_max
))
print('pearsons r: {0:0.3f}\npearsons rÂ²: {1:0.3f}\nfor {2} and {3} data sets'.format(
pearson_r_max_2, pearson_r_max_2 ** 2, c1_max_2, c2_max_2
))
...
plt.xlabel(c1_zero)
plt.ylabel(c2_zero)
plt.text(2.5e11, 30, (r'$r_{{pearson}}={0:0.3f}${2}$r_{{pearson}}^2={1:0.3f}$').format(
...
plt.close(2)
print('pearsons r: {0:0.3f}\npearsons rÂ²: {1:0.3f}\nfor {2} and {3} data sets'.format(
pearson_r_zero, pearson_r_zero ** 2, c1_zero, c2_zero
))
...
plt.xlabel(c1_min)
plt.ylabel(c2_min)
plt.text(10, 80, (r'$r_{{pearson}}={0:0.3f}${2}$r_{{pearson}}^2={1:0.3f}$').format(
...
plt.close(2)
print('pearsons r: {0:0.3f}\npearsons rÂ²: {1:0.3f}\nfor {2} and {3} data sets'.format(
pearson_r_min, pearson_r_min ** 2, c1_min, c2_min
))
