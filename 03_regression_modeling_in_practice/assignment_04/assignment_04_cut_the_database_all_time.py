...
for year in reversed(range(2006, 2016)):
    db.loc[db['latest_indicator'].isnull(), 'latest_indicator'] = db.loc[db['latest_indicator'].isnull(), str(year)]
    print('total data points after adding year {0}: {1}'.format(year, db['latest_indicator'].notnull().sum()))
...
db.to_csv('world_bank_selected_non_discriminating_based_on_gender_indicators.csv',
          columns=['country_code', 'series_code', 'latest_indicator'])