...
# have to delete them
sub1 = data[(data['AGE'] <= 25) & (data['CHECK321'] == 1) &
            (data['S3AQ3B1'] == 1) & (data['IDNUM'] != 20346) &
            (data['IDNUM'] != 36471) & (data['IDNUM'] != 28724)]

# subset data for logistic regression analyses (18-25, smoked in past month)
sub1 = data[(data['AGE'] <= 25) & (
    data['CHECK321'] == 1) & (data['S3AQ3B1'] == 1)]
...
# sum symptoms
sub1['CWITHDR_COUNT'] = numpy.nansum(
    [sub1['S3AQ8B7A'],
     sub1['S3AQ8B7B'],
     sub1['S3AQ8B7C'],
     sub1['S3AQ8B7D'],
...
# Current use continued despite knowledge of physical or psychological
# problem criteria #7 DSM-IV
...
# sum all symptoms (np.nansum allows rows with some missing values to
# count all valid values)
sub1['NDSymptoms'] = numpy.nansum(
    [sub1['crit1'],
     sub1['crit2'],
     sub1['S3AQ8B13'],
     sub1['crit4'],
...
sub1['numbercigsmoked_c'] = (
    sub1['numbercigsmoked'] -
    sub1['numbercigsmoked'].mean())
...
reg6 = smf.ols(
    'NDSymptoms ~ DYSLIFE + MAJORDEPLIFE + numbercigsmoked_c + age_c + SEX + C(ETHRACE)',
    data=sub1).fit()
print(reg6.summary())
...
reg7 = smf.ols(
    'NDSymptoms ~ DYSLIFE + MAJORDEPLIFE + numbercigsmoked_c + age_c + SEX + C(ETHRACE, Treatment(reference=1))',
    data=sub1).fit()
print(reg7.summary())
...
lreg2 = smf.logit(
    formula='NICOTINEDEP ~ SOCPDLIFE + MAJORDEPLIFE',
    data=sub1).fit()
print(lreg2.summary())
...
lreg4 = smf.logit(
    formula='NICOTINEDEP ~ PANIC + MAJORDEPLIFE',
    data=sub1).fit()
print(lreg4.summary())
